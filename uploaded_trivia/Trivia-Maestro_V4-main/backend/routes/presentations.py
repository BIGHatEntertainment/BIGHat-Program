...
async def import_trivia(request: TriviaImportRequest):
    ...
    try:
        ...
        # Extract location name from path (e.g., "01_Trivia/.../02_Locations/01_Monkey Pants" -> "Monkey Pants")
        location_folder = request.location.split('/')[-1] if request.location else "Unknown"
        location_name = location_folder
        # Remove numeric prefix (e.g., "01_Monkey Pants" -> "Monkey Pants")
        import re
        location_name_clean = re.sub(r'^\d+_', '', location_name)
        
        date_str = datetime.utcnow().strftime('%m/%d/%Y')
        presentation_name = request.presentationName or f"{location_name_clean} - {date_str}"
        
        # Extract host name from host file path (e.g., "01_Trivia/.../01_Hosts/Nick.pptx" -> "Nick")
        host_name = None
        if request.host:
            host_name = request.host.split('/')[-1].replace('.pptx', '') if '/' in request.host else request.host
        
        # Collect round names and types from the usage records
        round_names_list = [record['fileName'] for record in usage_records]
        round_types_list = [record['roundType'] for record in usage_records]
        
        from models import TriviaPresentation
        presentation = TriviaPresentation(
            name=presentation_name,
            createdBy=request.userName,
            location=location_name_clean,  # Clean location name for display
            locationFolder=location_folder,  # Full folder name for matching (e.g., "01_Monkey Pants")
            host=host_name,  # Host name for display
            hostFile=request.host,
            locationFile=location_file or "",
            roundFiles=round_files,
            roundNames=round_names_list,  # Store round names for Story Generator
