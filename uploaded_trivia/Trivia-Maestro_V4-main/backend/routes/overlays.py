...
from overlay_service import OverlayService, clear_overlay_caches
...
async def get_overlay_stats() -> Dict:
    """
    Get overlay processing statistics including Rust vs Python performance
    and LRU cache memory usage.
    """
    try:
        overlay_service = OverlayService()
        stats = overlay_service.get_processing_stats()
        ...
        logger.error(f"Error getting overlay stats: {str(e)}")
        return {
            'success': False,
            'error': str(e)
        }
...
async def clear_cache() -> Dict:
    """
    Clear all overlay caches (Python LRU and Rust) to free memory.
    Use this if memory usage is high or after processing many presentations.
    """
    try:
        # Clear caches
        ...
        logger.info(f"🧹 Cleared overlay caches and collected {collected} objects")
...
async def get_single_overlay_image(path: str) -> Dict:
    ...
    try:
        ...
        # If that failed, try to fix the location folder name (add numeric prefix)
        if not data_url and '/02_Locations/' in path:
            import re
            # Extract the location name from the path
            # Path format: .../02_Locations/{LocationName}/{filename}
            parts = path.split('/02_Locations/')
            if len(parts) == 2:
                location_and_file = parts[1].split('/')
                if len(location_and_file) >= 2:
                    location_name = location_and_file[0]
                    filename = '/'.join(location_and_file[1:])
                    
                    # If location doesn't have a numeric prefix, try to find it
                    if not re.match(r'^\d+_', location_name):
                        logger.info(f"🔍 Trying to find location folder for: {location_name}")
                        
                        # List location folders to find the right one
                        sp = SharePointService()
                        base_path = "01_Trivia/Web App/00_Builder/02_Locations"
                        try:
                            folders = sp.list_folder_contents(base_path)
                            for folder in folders:
                                if folder.get('folder'):
                                    folder_name = folder['name']
                                    # Check if this folder matches (with or without prefix)
                                    clean_name = re.sub(r'^\d+_', '', folder_name).lower()
                                    if clean_name == location_name.lower():
                                        # Found the matching folder!
                                        corrected_path = f"{parts[0]}/02_Locations/{folder_name}/{filename}"
                                        logger.info(f"✅ Found correct path: {corrected_path}")
                                        data_url = overlay_service.get_overlay_as_data_url(corrected_path, overlay_type)
                                        break
                        except Exception as e:
                            logger.warning(f"Failed to search for location folder: {e}")
...
async def apply_section_overlays(request: ApplySectionOverlaysRequest) -> Dict:
    ...
    try:
        ...
        logger.info("✅ All overlays pre-loaded")
