...
## 2026-02-27 — v32.0.0-alpha.19: HOTFIX — offline-mode entitlement grant

### What broke in alpha.18
Customer hit the "We don't recognise that license key" error in the
Setup Wizard because the cloud DB had been wiped during a redeploy and
the env-var bootstrap-restore silently failed twice (placeholder email,
then bootstrap code apparently not running in production). Workaround
the wizard offered — **"Continue offline"** — would complete setup BUT
left `subscription.owns_standalone=false`, so the dashboard's Trivia
card stayed permanently locked with "Complete first-run setup to
unlock." This defeated the entire purpose of the offline-tolerant path
and meant a paid customer with a wiped cloud DB couldn't use the app
they'd already bought.

### Root cause
`backend/native/router.py::initialize_setup()` had two branches for the
final state-mirroring step:
```python
if pending_cloud:
    # offline_mode OR cloud transport error
    lic["pending_cloud_activation"] = True
    register_seat(...)           # ← only this, no _apply_cloud_response_to_local_state
else:
    _apply_cloud_response_to_local_state(cloud_resp, ...)   # sets owns_standalone
    register_seat(...)
```
The `pending_cloud` branch wrote the license KEY string but never set
`owns_standalone`. The customer was stuck.

### Fix (alpha.19)
The `pending_cloud` branch now synthesises a provisional-grant cloud
response and runs `_apply_cloud_response_to_local_state(...)` against
it, mirroring the standalone-tier defaults into local state:
```python
_apply_cloud_response_to_local_state(
    {"ok": True, "owns_standalone": True, "owns_music_bingo": False,
     "owns_karaoke": False, "cloud_library_active": False,
     "max_seats": 5, "active_seats": 1, "pending": True},
    license_key=key, email=admin_email,
)
```
`pending_cloud_activation=true` is still set so the existing 4-hour
refresh job knows to keep retrying the cloud. When cloud catches up,
the next refresh either confirms entitlement or downgrades — same
optimistic-then-reconcile pattern every offline-first app uses.

### Security note
This grant happens ONLY after `is_well_formed_license(key)` passes
(checked at line 167 before we reach the pending branch). A random
gibberish string still gets rejected upfront. We're not opening the
door to "any string unlocks the app" — we're letting a SYNTACTICALLY
valid BHE-XXXX-XXXX-XXXX-XXXX key grant standalone-tier features
offline pending cloud reconciliation. If the cloud later rules the key
unknown/revoked, the refresh job downgrades the local state.

### Files of reference for the NEXT agent
- `/app/backend/native/router.py` — `initialize_setup` lines ~246–290.
  Read the inline comment "v32.0.0-alpha.19: optimistic offline grant"
  before touching this branch again.
- `/app/backend/tests/test_offline_setup_grants_entitlement.py` —
  2 pytest tests pinning the grant. **Don't delete or skip.**

### Customer recovery path (no env-var dance required)
1. Install alpha.19 over alpha.18 (preserves data via in-place update).
2. If the dashboard is still locked: click the user dropdown →
   "Enter License Key" → enter the key + email → click Activate.
   Cloud will still 404 (DB wipe), but the wizard's separate
   "Continue offline" button (when on a fresh install) IS the
   bulletproof path — completes setup, unlocks Trivia immediately,
   background job reconciles with cloud later.
3. To get a fully-online install on alpha.19: uninstall via Apps &
   Features (this wipes %LOCALAPPDATA%\\BIGHat\\), reinstall, and on
   the Setup Wizard's License step click **Continue offline**.
   Standalone features unlock immediately.

### Verification
- Backend pytest contract: 19/19 PASS (added 2 new tests).
- Real-world Windows verification requires the v32.0.0-alpha.19
