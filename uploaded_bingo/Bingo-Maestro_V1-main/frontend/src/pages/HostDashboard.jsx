...
import { BingoBall, BingoBoard, MusicBingoBall } from "../components/BingoComponents";
...
export default function HostDashboard() {
  ...
  const callNextSong = async () => {
    ...
    if (song) {
      ...
      try {
        await axios.post(`${API}/game/call-song`, {
          number: nextNumber,
          title: song.title,
          artist: song.artist
        });
  ...
  const claimBingo = async () => {
    try {
      await axios.post(`${API}/game/bingo`);
      stopTimer();
      if (videoRef.current) {
        videoRef.current.pause();
        setIsPlaying(false);
  ...
  const verifyBingo = async (confirmed) => {
    try {
      await axios.post(`${API}/game/verify-bingo`, {
  ...
  const endRound = async () => {
    try {
      await axios.post(`${API}/game/end-round`);
      stopTimer();
  ...
  const newRound = async () => {
    try {
      await axios.post(`${API}/game/new-round`);
      setCurrentSong(null);
      setNextSong(null);
      setCalledSongs([]);
