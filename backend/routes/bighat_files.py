...
# ============================================================
# Play Direct — alpha.25
# ============================================================
#
# Lets the host pick a `.bighat` ROUND file from the Trivia Presenter
# dashboard and play it immediately, skipping the Round Maker / Build
# Wizard. The file goes through the SAME normalisation pipeline as a
# regular .bighat import (so the round renders identically to one the
# host built locally), then gets compiled into the same `.pptx` shape
# the Trivia Presenter normally consumes, and is wrapped in a
# lightweight `trivia_presentations` doc so the in-app presenter view
# can open it.
#
# Why a separate endpoint instead of overloading /import?
#   1. /import is also called from Round Maker — adding presentation-
#      creation side-effects there would surprise that flow.
#   2. Play-direct has a different success contract — it returns the
#      *presentation* id (not the round id) so the frontend can
#      navigate straight to /trivia/present?id=… without an extra
#      lookup.
#   3. We want to fail loud on non-round .bighat files (e.g. a host
#      accidentally drops a bingo card) instead of silently importing
#      it into the wrong collection.

class PlayDirectResult(BaseModel):
    presentation_id: str
    round_id: str
    name: str
    round_type: str
...
@router.post("/play-direct", response_model=PlayDirectResult)
async def play_direct_bighat(file: UploadFile = File(...)):
    """One-click pipeline: .bighat → round in DB → .pptx → single-round
    trivia presentation → presentation_id the host can present.

    Reuses the canonical `_import_zip_bytes` translator (alpha.23/24)
    so question shape, options, correct answers, and the bundled title
    card all land the same way they would when the host imports via
    the Round Maker dashboard. The generated .pptx is the same one
    the Round Maker `/rounds/{id}/generate` endpoint emits — there is
    no second formatting code path."""
    payload = await file.read()
    result = await _import_zip_bytes(payload)
    if result.type != "round":
        raise HTTPException(
            400,
            f"Play-direct only supports round .bighat files (got '{result.type}'). "
            f"Use the regular import for presentations / bingo / scoreboard.",
        )

    # Pull the canonical round doc back out — `_import_zip_bytes` has
    # already run the normalisers + cover-image ingest.
    round_doc = await db.rounds.find_one({"id": result.id}, {"_id": 0})
    if not round_doc:                              # pragma: no cover
        raise HTTPException(500, "Round import succeeded but doc not found")

    # Compile to .pptx via the same generator the Round Maker dashboard
    # uses. Lazy-import so this module stays independent of roundmaker's
    # heavy python-pptx import chain unless play-direct is actually hit.
    try:
        try:
            from routes.roundmaker import generate_pptx
        except ModuleNotFoundError:
            from backend.routes.roundmaker import generate_pptx
        pptx_path = generate_pptx(round_doc)
    except Exception as e:
        logger.error("[play-direct] PPTX generation failed: %s", e)
        raise HTTPException(500, f"Failed to generate slides: {e}")

    # Persist the generated path on the round so re-presenting doesn't
    # regenerate (and so Round Maker sees the round as "generated").
    await db.rounds.update_one(
        {"id": result.id},
        {"$set": {"pptx_path": pptx_path, "status": "generated"}},
    )

    # Wrap in a single-round trivia presentation. `type='trivia-imported'`
    # is what /trivia-viewer/list filters on and what the editor checks
    # before requesting per-slide rendering via /slides.
    pres_id = str(uuid.uuid4())
    round_type = round_doc.get("round_type", "MC")
    round_name = round_doc.get("name", "Round")
    presentation_doc = {
        "id": pres_id,
        "name": round_name,
        "type": "trivia-imported",
        "createdAt": datetime.now(timezone.utc).isoformat(),
        "createdBy": "Direct Play",
        "host": "Direct Play",
        "location": "Direct Play",
        "numRounds": 1,
        "totalSlides": 0,                          # slides endpoint computes on demand
        "roundTypes": [round_type],
        "roundNames": [round_name],
        "roundFiles": [{
            "order": 1,
            "file": pptx_path,                     # local absolute path; slides
                                                   # endpoint short-circuits the
                                                   # SharePoint download when the
                                                   # path exists locally.
            "name": round_name,
            "type": round_type,
            "slideCount": 12,                      # estimate; recomputed on render
        }],
        "direct_play": True,                       # marker for diagnostics
        "direct_play_round_id": result.id,         # traceability
        "imported_from": ".bighat",
    }
    await db.trivia_presentations.insert_one(presentation_doc)
    logger.info(
        "[play-direct] wrapped round %s (%s) as presentation %s",
        result.id, round_type, pres_id,
    )

    return PlayDirectResult(
