...
class Element(BaseModel):
    ...
    type: str  # 'text' | 'image' | 'shape' | 'overlay' | 'video'
    ...
    videoSrc: Optional[str] = None  # for videos (base64 data URL)
    mimeType: Optional[str] = None  # video/mp4, video/webm
...
class TriviaPresentation(BaseModel):
    ...
    createdBy: str
    createdAt: datetime = Field(default_factory=datetime.utcnow)
    location: str  # Location name (display name)
    locationFolder: Optional[str] = None  # Full location folder name (e.g., "01_Monkey Pants")
    host: Optional[str] = None  # Host name (display name)
    hostFile: str  # Host file path
    locationFile: str  # Location intro file path
    roundFiles: List[dict]  # [{order, type, file}]
    roundNames: Optional[List[str]] = None  # Round names for Story Generator
    roundTypes: Optional[List[str]] = None  # Round types (MC, REG, MISC, etc.)
