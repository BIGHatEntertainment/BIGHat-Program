...
class SharePointService:
    ...
    def __init__(self):
        ...
        # SharePoint configuration - use environment variables with defaults
        self.site_url = os.environ.get('SHAREPOINT_SITE_URL', 'https://bhentertainment.sharepoint.com')
        self.site_name = os.environ.get('SHAREPOINT_SITE_NAME', 'bhentertainment.sharepoint.com')
        # The base folder path in SharePoint
        self.base_folder = os.environ.get('SHAREPOINT_BASE_FOLDER', '01_Trivia/Web App/00_Builder')
    ...
    def get_site_id(self) -> str:
        ...
        url = 'https://graph.microsoft.com/v1.0/sites/bhentertainment.sharepoint.com'
    ...
    def get_item_by_id(self, item_id: str) -> Dict:
        """Get item metadata by SharePoint item ID.
        
        This is useful for resolving item IDs stored in presentations back to their actual names.
        
        Args:
            item_id: The SharePoint item ID (e.g., '01Z4PLCYWELSHNUKQYOBFJUJKNMUR67DFV')
            
        Returns:
            Dict with item metadata including 'name', 'id', etc.
        """
        try:
            token = self.get_access_token()
            headers = {'Authorization': f'Bearer {token}'}
            ...
            if response.status_code == 200:
                ...
                logger.error(f"Failed to get item {item_id}: {response.text}")
                return {}
        except Exception as e:
    ...
    def upload_content(self, content: bytes, sharepoint_path: str, content_type: str = 'application/octet-stream') -> bool:
        """Upload content directly to SharePoint without a local file
        
        Args:
            content: Raw bytes to upload
            sharepoint_path: Full path in SharePoint (e.g., '01_Trivia/Web App/00_Builder/02_Locations/01_Monkey Pants/00_Built/file.json')
        ...
        try:
            ...
            response = requests.put(url, headers=headers, data=content, timeout=SHAREPOINT_TIMEOUT)
            
            if response.status_code in [200, 201]:
                logger.info(f"Successfully uploaded to {sharepoint_path}")
                return True
            else:
                logger.error(f"Failed to upload content to {sharepoint_path}: {response.text}")
                return False
        except Exception as e:
            logger.error(f"Error uploading content to {sharepoint_path}: {str(e)}")
            return False
