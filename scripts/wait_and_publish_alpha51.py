#!/usr/bin/env python3
"""Wait for alpha.51 build → auto-cancel Intel leg → publish PUBLIC + LATEST
→ sweep any prior lingering DRAFT releases and publish them (the merchant
does not want DRAFT releases sitting around).
"""
...
# NOTE (alpha.50 lesson learned): we DO NOT fall back to /runs/{id}/cancel
...
_, releases = gh("GET", "/releases?per_page=100")
rel = next((r for r in (releases or []) if r["tag_name"] == TAG), None)
if not rel:
...
# --------------------------------------------------------------
# Sweep DRAFT releases — the merchant does not want drafts
# sitting around. Publish them (draft=false) but keep them as
# prereleases so they don't override /releases/latest.
# --------------------------------------------------------------
print("\n→ Sweeping DRAFT releases…", flush=True)
_, all_rels = gh("GET", "/releases?per_page=100")
swept = 0
for r in (all_rels or []):
    if r["id"] == rel["id"]:
        continue
    if r.get("draft"):
        code, _ = gh("PATCH", f"/releases/{r['id']}", {
            "draft": False, "prerelease": True, "make_latest": "false",
        })
        if code in (200, 201):
            print(f"  ✓ Published draft: {r['tag_name']}")
            swept += 1
        else:
            print(f"  ⚠ Failed to publish draft {r['tag_name']}: HTTP {code}")
print(f"→ Swept {swept} draft(s). {TAG} is LATEST.", flush=True)
