#!/usr/bin/env python3
"""Push v32.0.0-alpha.52. Kill "The Clue" hallucination + prototype
provenance flags + /api/native/attest endpoint."""
...
FILES = [
    ("/app/backend/VERSION.txt", "backend/VERSION.txt",
     "chore(release): bump -> 32.0.0-alpha.52"),
    ("/app/src-tauri/tauri.conf.json", "src-tauri/tauri.conf.json",
     "chore(release): bump tauri.conf.json -> 32.0.0-alpha.52"),
    ("/app/backend/native_slides.py", "backend/native_slides.py",
     "fix(alpha.52): delete hallucinated 'The Clue' header on BIG round; "
     "stamp _verified_from_prototype metadata on every slide; add "
     "_title_card_source provenance; add per-category REG title-card lookup"),
    ("/app/backend/native/router.py", "backend/native/router.py",
     "feat(alpha.52): GET /api/native/attest/{presentation_id} — full "
     "per-slide provenance + health report"),
    ("/app/backend/tests/test_alpha52_no_the_clue_and_attestation.py",
     "backend/tests/test_alpha52_no_the_clue_and_attestation.py",
     "test(alpha.52): 5 tests locking in the removal + provenance stamps"),
    ("/app/memory/CHANGELOG.md", "memory/CHANGELOG.md",
     "docs(alpha.52): changelog entry"),
]
