#!/usr/bin/env python3
"""Push v32.0.0-alpha.48. Round-fix, audience window, host/location images."""
...
FILES = [
    ("/app/backend/VERSION.txt", "backend/VERSION.txt",
     "chore(release): bump -> 32.0.0-alpha.48"),
    ("/app/src-tauri/tauri.conf.json", "src-tauri/tauri.conf.json",
     "chore(release): bump tauri.conf.json -> 32.0.0-alpha.48"),
    ("/app/src-tauri/capabilities/default.json", "src-tauri/capabilities/default.json",
     "feat(alpha.48): permit WebviewWindow creation + trivia-audience window"),
    ("/app/backend/native_slides.py", "backend/native_slides.py",
     "fix(alpha.48): trust file-path source of truth; render host 9:16 + location branding + title-cards"),
    ("/app/backend/routes/slide_fetcher.py", "backend/routes/slide_fetcher.py",
     "fix(alpha.48): sections-list enforces canonical MC->REG->MISC->MYS->BIG order"),
    ("/app/frontend/src/components/trivia/editor/PresentationMode.jsx",
     "frontend/src/components/trivia/editor/PresentationMode.jsx",
     "fix(alpha.48): openAudienceView uses dynamic @tauri-apps/api/webviewWindow (window.__TAURI__ isn't injected in Tauri v2)"),
    ("/app/frontend/src/pages/trivia/TriviaAudienceView.jsx",
     "frontend/src/pages/trivia/TriviaAudienceView.jsx",
     "feat(alpha.48): Final Scores CSS scrolling credits animation"),
    ("/app/backend/tests/test_alpha48_round_fix_and_assets.py",
     "backend/tests/test_alpha48_round_fix_and_assets.py",
     "test(alpha.48): 13 regression tests including MC-02-A merchant repro"),
    ("/app/memory/CHANGELOG.md", "memory/CHANGELOG.md",
     "docs(alpha.48): changelog entry"),
]
