#!/usr/bin/env python3
"""Push v32.0.0-alpha.51. `.bighat` ZIP extraction + 16:9 host image priority."""
...
FILES = [
    ("/app/backend/VERSION.txt", "backend/VERSION.txt",
     "chore(release): bump -> 32.0.0-alpha.51"),
    ("/app/src-tauri/tauri.conf.json", "src-tauri/tauri.conf.json",
     "chore(release): bump tauri.conf.json -> 32.0.0-alpha.51"),
    ("/app/backend/native_slides.py", "backend/native_slides.py",
     "fix(alpha.51): unzip .bighat ZIP archives; normalise payload keys "
     "(prompt/n/correct_index); inline embedded cover_image + per-Q media "
     "as data URLs; prefer host 16:9 over 9:16"),
    ("/app/backend/tests/test_alpha51_bighat_zip_and_host_1610.py",
     "backend/tests/test_alpha51_bighat_zip_and_host_1610.py",
     "test(alpha.51): 7 tests locking in ZIP extraction + 16:9 host priority"),
    ("/app/memory/CHANGELOG.md", "memory/CHANGELOG.md",
     "docs(alpha.51): changelog entry"),
]
