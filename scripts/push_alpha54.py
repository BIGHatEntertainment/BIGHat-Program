#!/usr/bin/env python3
"""Push v32.0.0-alpha.54. REG round title cards from roundmaker_uploads/."""
...
FILES = [
    ("/app/backend/VERSION.txt", "backend/VERSION.txt",
     "chore(release): bump -> 32.0.0-alpha.54"),
    ("/app/src-tauri/tauri.conf.json", "src-tauri/tauri.conf.json",
     "chore(release): bump tauri.conf.json -> 32.0.0-alpha.54"),
    ("/app/backend/native_slides.py", "backend/native_slides.py",
     "fix(alpha.54): REG round title cards inline from "
     "backend/roundmaker_uploads/<cover_image_id>.<ext> — the same "
     "image the round-editor preview shows"),
    ("/app/backend/tests/test_alpha54_reg_title_from_roundmaker_uploads.py",
     "backend/tests/test_alpha54_reg_title_from_roundmaker_uploads.py",
     "test(alpha.54): 7 tests locking in cover_image_id → data-URL "
     "inlining including the merchant's real animals-1 regression"),
    ("/app/memory/CHANGELOG.md", "memory/CHANGELOG.md",
     "docs(alpha.54): changelog entry"),
]
