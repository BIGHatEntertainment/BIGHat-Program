#!/usr/bin/env python3
"""Wait for alpha.39 build to finish, then publish."""
...
while True:
    ...
    mac_done = mac and mac["status"] == "completed"
    if win_done and mac_done: break
    if elapsed > 80:
        print("!! timeout waiting"); sys.exit(1)
    time.sleep(45)
...
print(f"\nPATCH → {code}, draft={res.get('draft')}")
