#!/usr/bin/env python3
"""Wait for alpha.40 build → publish PUBLIC + LATEST.

Locked policy: every release is public (`draft:false`, `prerelease:false`)
and marked as LATEST (`make_latest:"true"`) so the merchant's download
page always points at the newest build.
"""
...
print(f"\n→ Release id={rel['id']} draft={rel['draft']} assets ({len(rel['assets'])}):")
for a in rel["assets"]:
...
# Policy: draft=false + prerelease=false + make_latest=true
...
# Verify /releases/latest actually points at this tag now
