#!/usr/bin/env python3
"""Push + publish v32.0.0-alpha.44. Public + latest."""
...
FILES = [
    ("/app/backend/VERSION.txt", "backend/VERSION.txt",
     "chore(release): bump -> 32.0.0-alpha.44"),
    ("/app/src-tauri/tauri.conf.json", "src-tauri/tauri.conf.json",
     "chore(release): bump tauri.conf.json -> 32.0.0-alpha.44"),
    ("/app/backend/routes/trivia_import.py", "backend/routes/trivia_import.py",
     "feat(alpha.44): /trivia-import slides + chunk + metadata delegate to native _assemble_slides_native"),
    ("/app/frontend/src/pages/trivia/TriviaPresenterView.jsx",
     "frontend/src/pages/trivia/TriviaPresenterView.jsx",
     "fix(alpha.44): restore prototype behavior — Present Live opens the full Editor"),
    ("/app/memory/PRD.md", "memory/PRD.md",
     "docs(alpha.44): changelog entry"),
]
