...
@router.get("/list")
async def list_trivia_presentations(userName: str) -> List[Dict]:
    """List all trivia presentations for a user"""
    try:
...
async def get_trivia_presentation(presentation_id: str) -> Dict:
    """
    Get trivia presentation details including location information.
    Used by the editor to fetch location for overlay functionality.
    """
    try:
        ...
        # Return presentation with location info
        return {
            "id": presentation['id'],
