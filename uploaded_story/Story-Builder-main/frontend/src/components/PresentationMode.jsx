...
import { X, ChevronLeft, ChevronRight, Monitor, ListOrdered, Pause, Play, Eye } from 'lucide-react';
import { Button } from './ui/button';

const PresentationMode = ({ slides, onExit, onOpenScoreTracker, presentationId, isScoreTrackerOpen = false, overlayCache }) => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [audienceIndex, setAudienceIndex] = useState(0); // Separate index for audience
  const [audienceWindow, setAudienceWindow] = useState(null);
  const [isSyncEnabled, setIsSyncEnabled] = useState(true); // Auto-sync by default
  ...
  const [revealedAnswers, setRevealedAnswers] = useState({}); // Track revealed answers per slide {slideIndex: count}
  const [presenterScale, setPresenterScale] = useState(1); // Scale factor for presenter view based on resolution
  ...
  const getFinalScores = useCallback(() => {
    ...
    try {
      ...
      const teamsWithScores = teams
        ...
        .map(team => {
          const roundScores = team.rounds.slice(0, currentRounds.length).map((score, idx) => {
            const points = parseInt(score) || 0;
            return points * currentRounds[idx].multiplier;
          });
  ...
  const updateAudienceView = useCallback((index) => {
    if (audienceWindowRef.current && !audienceWindowRef.current.closed) {
      try {
        ...
        // MEMORY OPTIMIZATION: Resolve overlay references to actual data before sending
        // This avoids duplicating base64 data in React state while still sending to audience
          ...
          resolvedSlide = {
            ...
            elements: slide.elements.map(el => {
              if (el.type === 'overlay' && el.overlayId) {
                // Convert overlay reference to image with actual data
                return {
                  ...el,
                  type: 'image',
    ...
    if (newWindow) {
          ...
          <script>
            ...
            function renderSlide(slide, revealedCount = 0, isAnswerSlide = false, finalScoresData = null) {
              ...
              if (isFinalScoresSlide && finalScoresData && finalScoresData.teams.length > 0) {
                // Render final scores overlay with auto-scrolling - 16:9 fullscreen layout
                const scoresHTML = \`
                  <style>
                    ...
                    .scroll-container {
                      ...
                      width: 100%;
                      height: 100%;
                      padding: 0 5%; /* 5% padding on left and right sides */
                      box-sizing: border-box;
                    }
                    .scroll-header {
                      flex-shrink: 0;
                  ...
                  <div class="scroll-container">
                    <div class="scroll-header">
                      <h2 style="font-size: 4rem; font-weight: bold; color: #FFD700; text-align: center; font-family: Lemonada, cursive;">
                        🏆 Final Scores 🏆
                      </h2>
                    </div>
                    <div class="scroll-content-wrapper">
                      <div class="scroll-content">
                        \${finalScoresData.teams.map((team, idx) => \`
                          <div class="team-card" style="background: linear-gradient(to right, \${idx === 0 ? 'rgba(255,215,0,0.35), rgba(255,165,0,0.35)' : idx === 1 ? 'rgba(192,192,192,0.35), rgba(169,169,169,0.35)' : idx === 2 ? 'rgba(205,127,50,0.35), rgba(160,82,45,0.35)' : 'rgba(0,0,139,0.35), rgba(0,0,70,0.35)'}); border: 3px solid \${idx === 0 ? '#FFD700' : idx === 1 ? '#C0C0C0' : idx === 2 ? '#CD7F32' : '#0066CC'};">
                            <div class="team-info">
                              <div style="display: flex; align-items: center; gap: 2rem;">
                                <span class="team-rank">\${idx + 1}.</span>
                                <div>
                                  <h3 class="team-name">\${team.name}</h3>
                                  \${team.swag ? \`<p style="font-size: 1.1rem; color: #ccc;">\${team.swag}</p>\` : ''}
                                </div>
                              </div>
                              <div style="text-align: right;">
                                <p class="team-total">\${team.total}</p>
                                <p style="font-size: 1rem; color: #999;">Total Points</p>
                              </div>
                            </div>
                        ...
                        <div style="margin-top: 3rem;">
                          <h2 style="font-size: 4rem; font-weight: bold; color: #FFD700; text-align: center; margin-bottom: 2rem; font-family: Lemonada, cursive;">
                            🏆 Final Scores 🏆
                          </h2>
                        </div>
                        \${finalScoresData.teams.map((team, idx) => \`
              ...
              if (slide.elements) {
                ...
                // 3) BIG Question (slide 2) and BIG review (slide 4): +10% (1.10)
                ...
                if (isWinnersSlide) {
                  ...
                  // BIG round: slide 2 (question) and slide 4 (review): +10%
            ...
            function goFullscreen() {
              ...
              if (elem.requestFullscreen) {
                elem.requestFullscreen().then(function() {
                  ...
                  // Keep prompt visible if it fails
                });
      ...
      setTimeout(() => {
        try {
          ...
          console.log('✅ Audience window moved to right display:', rightDisplayX, 0);
        } catch (error) {
          console.warn('Could not move window to right display:', error);
        }
      }, 100);
      ...
      <div className="w-full h-full flex items-center justify-center p-8">
        ...
        >
          {currentSlide.elements.map((element) => {
            ...
            return (
              ...
              >
                ...
                {element.type === 'overlay' && overlayCache?.current && (
          ...
          {currentSlide?.metadata?.roundType === 'WINNERS' && currentSlide?.metadata?.slideIndexInRound === 4 && (() => {
            ...
            if (!scoresData || scoresData.teams.length === 0) return null;
            
            return (
              <div className="absolute inset-0 flex items-center justify-center z-20" style={{ aspectRatio: '16/9', padding: '0 5%' }}>
                <div className="bg-black/85 backdrop-blur-md w-full h-full p-6 overflow-hidden flex flex-col">
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-6">
                            ...
                            <div>
                              ...
                              {team.swag && <p className="text-lg text-gray-300">{team.swag}</p>}
                            </div>
                          </div>
                          <div className="text-right">
                            <p className="text-5xl font-bold text-yellow-400" style={{ fontFamily: 'Lemonada, cursive' }}>{team.total}</p>
                        ...
                        <div className="flex gap-3 mt-4 flex-wrap">
                          {team.roundScores.map((score, roundIdx) => (
                            <div key={roundIdx} className="bg-black/50 px-4 py-2 rounded-lg">
                              <span className="text-sm text-gray-400">{scoresData.rounds[roundIdx].label}:</span>
                              <span className="text-lg font-bold text-white ml-2">{score}</span>
                            </div>
                          ))}
                        </div>
                      </div>
