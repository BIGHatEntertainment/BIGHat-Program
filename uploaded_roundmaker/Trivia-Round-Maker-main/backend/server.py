...
class QuestionItem(BaseModel):
    ...
    question: str
    answer: str
    options: Optional[List[str]] = None  # For MC: [A, B, C, D]
    correctOption: Optional[int] = None  # For MC: index of correct option (0-3)
...
class RoundCreate(BaseModel):
    round_type: str  # MC, REG, MISC, MYS, BIG
    name: str
    questions: List[QuestionItem]
    tiebreaker: Optional[TiebreakerItem] = None  # BIG only
...
@api_router.post("/rounds/{round_id}/duplicate", response_model=RoundResponse)
...
@api_router.post("/rounds/{round_id}/generate")
...
@api_router.get("/reg-title-images")
...
@api_router.get("/reg-next-number/{category}")
...
@api_router.post("/reg-download-title-image")
...
@api_router.get("/sharepoint-status")
...
@api_router.post("/rounds/{round_id}/upload-sharepoint")
