...
export default function Dashboard() {
  ...
  const handleUploadSharepoint = async (round) => {
    ...
    try {
      const res = await axios.post(`${API}/rounds/${round.id}/upload-sharepoint`);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${API}/rounds/${id}`);
  ...
  const handleDownload = async (round) => {
    try {
      const res = await axios.post(`${API}/rounds/${round.id}/generate`, null, {
        responseType: "blob",
      });
  ...
  const handleDuplicate = async (round) => {
    try {
      const res = await axios.post(`${API}/rounds/${round.id}/duplicate`);
