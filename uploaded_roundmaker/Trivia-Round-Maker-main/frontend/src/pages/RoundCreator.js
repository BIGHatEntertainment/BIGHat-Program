...
export default function RoundCreator() {
  ...
  const handleSelectCategory = async (categoryName) => {
    ...
    try {
      ...
      setCoverPreview(`${API}/uploads/${encodeURIComponent(img.name)}`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await axios.post(`${API}/rounds`, payload);
  ...
  const handleSaveAndGenerate = async () => {
    ...
    try {
      const payload = buildPayload();
      const saveRes = await axios.post(`${API}/rounds`, payload);
      const roundId = saveRes.data.id;
      const genRes = await axios.post(`${API}/rounds/${roundId}/generate`, null, {
        responseType: "blob",
      });
  ...
  const handleSaveAndUpload = async () => {
    ...
    try {
      ...
      const saveRes = await axios.post(`${API}/rounds`, payload);
      const roundId = saveRes.data.id;
      const uploadRes = await axios.post(`${API}/rounds/${roundId}/upload-sharepoint`);
